    ...
    try:
        ...
        if operation == 'add':
            ...
            updated_members = existing_members + [
                m for m in members 
                if normalize_username(m) not in normalized_existing
            ]
        elif operation == 'remove':
            # Remove specified members
            updated_members = [
                m for m in existing_members 
                if normalize_username(m) not in normalized_new
            ]
        elif operation == 'sync':
                ...
                if any(normalize_username(m).startswith(sub_team_name.split('-')[-1]) for m in members):
                    ...
                    if operation == 'add':
                        ...
                        sub_team['members'] = existing_sub_members + [
                            m for m in members 
                            if m not in existing_sub_members and 
                            normalize_username(m).startswith(sub_team_name.split('-')[-1])
                        ]
                    elif operation == 'remove':
                        sub_team['members'] = [
                            m for m in sub_team.get('members', [])
                            if normalize_username(m) not in normalized_new
                        ]
                    elif operation == 'sync':
                        sub_team['members'] = [
                            m for m in members 
                            if normalize_username(m).startswith(sub_team_name.split('-')[-1])
                        ]
