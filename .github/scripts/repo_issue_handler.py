...
class RepositoryHealthChecker:
    ...
    def _check_branch_protection(self, repo, health_details: Dict[str, Any]):
        ...
        health_details["branch_protection"] = {"main_branch_protected": False, "protection_rules": {}}
    ...
    def _check_pr_reviews(self, branch_protection, required_rules):
        ...
        pr_review_rules = next(
            (rule.get("pull_request_reviews") for rule in required_rules if "pull_request_reviews" in rule), None
        )
    ...
    def _check_status_checks(self, branch_protection, required_rules):
        ...
        status_check_rules = next(
            (rule.get("status_checks") for rule in required_rules if "status_checks" in rule), None
        )
    ...
    def _check_up_to_date_branch(self, branch_protection, required_rules):
        ...
        up_to_date_rule = next(
            (rule.get("require_up_to_date_branch") for rule in required_rules if "require_up_to_date_branch" in rule),
            None,
        )
    ...
    def _check_admin_enforcement(self, branch_protection, required_rules):
        ...
        admin_rule = next((rule.get("enforce_admins") for rule in required_rules if "enforce_admins" in rule), None)
    ...
    def _check_push_restrictions(self, branch_protection, required_rules):
        ...
        push_restriction_rule = next(
            (rule.get("restrict_pushes") for rule in required_rules if "restrict_pushes" in rule), None
        )
